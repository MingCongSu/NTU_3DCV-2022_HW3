...
class Calibrator:
    def __init__(self, args):
        ...
        self.objp[:, :2] = np.mgrid[0:self.inner_h,
                                    0:self.inner_w].T.reshape(-1, 2)
    ...
    def run(self):
        ...
        self.load_images()
        assert len(self.imgs) >= 4, print(
            '=> Error: need a least 4 images to calibrate')
        ...
        if args.show:
            self.show_result(self.imgs, self.imgpoints, self.K,
                             self.dist, self.rvec, self.tvecs)
    ...
    def calibrate(self):
        ...
        for img in self.imgs:
            ...
            ret, corners = cv.findChessboardCorners(
                gray, (self.inner_h, self.inner_w), None)
            ...
            if ret == True:
                ...
                cv.drawChessboardCorners(
                    img_, (self.inner_h, self.inner_w), corners2, ret)
                cv.imshow('img', img_)
                cv.waitKey(45)
        ...
        assert len(self.objpoints) >= 4, print(
            '=> Error: need a least 4 images to calibrate')
    ...
    def show_result(self, imgs, imgpoints, mtx, dist, rvecs, tvecs):
        ...
        def create_camera(_img, _r_mat, _t_vec, _K):
            ...
            verts[4, 0] = _w / 2
            verts[4, 1] = _h / 2
            verts = (expand_batch(np.linalg.inv(_K), 5)
                     @ np.expand_dims(verts, -1))
            verts[4, -1] = 0.
            ...
            extrinsic = np.concatenate(
                [extrinsic, np.zeros([1, 4], np.float32)], axis=0)
            extrinsic[-1, -1] = 1.
            ...
            img_ = o3d.geometry.PointCloud.create_from_rgbd_image(
                ...
                o3d.camera.PinholeCameraIntrinsic(
                    _w, _h, _K[0, 0], _K[1, 1], _K[0, 2], _K[1, 2])
            )
        ...
        for i, (img, rvec, tvec, imgpoint) in enumerate(zip(imgs, rvecs, tvecs, imgpoints)):
            ...
            imgpoint_ = np.concatenate(
                [imgpoint, np.ones([imgpoint.shape[0], 1, 1], np.float32)], -1)
            ...
            line.lines = o3d.utility.Vector2iVector(
                [[i, i+1] for i in range(num_points - 1)])
            LINE_COLORS = [[1, 0, 0], [1, 0.5, 0], [0.75, 0.75, 0], [
                0, 1, 0], [0, 0.75, 0.75], [0, 0, 1], [1, 0, 1]]
